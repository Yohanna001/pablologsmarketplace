...
export default function App() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAFAFC]">
      ...
      {/* FLOATING FAST-TESTER UTILITY ACTION BUTTON */}
      <div className="fixed bottom-4 left-4 z-50">
        <div className="relative">
          <button
            id="tester-utils-trigger"
            onClick={() => setShowTesterMenu(!showTesterMenu)}
            className="px-4 py-3 bg-[#1A1A2E] hover:bg-[#16213E] text-white rounded-full border border-[#0F3460]/20 shadow-xl flex items-center gap-2 text-xs font-bold transition-all duration-300 hover:scale-105 active:scale-95 group cursor-pointer"
          >
            <Database className="w-4 h-4 text-[#0F3460] group-hover:rotate-12 transition-transform" />
            <span>🧪 Fast-Tester Helper</span>
            <span className="w-2 h-2 rounded-full bg-[#E94560] animate-ping absolute top-0.5 right-0.5" />
          </button>

          {showTesterMenu && (
            <div id="tester-utils-popover" className="absolute bottom-14 left-0 w-64 bg-white border border-[#E0E0E0] rounded-2xl shadow-2xl p-4 space-y-3 animate-[slideUp_0.2s_ease] text-xs">
              <div className="border-b pb-1.5 mb-1 flex items-center justify-between">
                <span className="font-bold text-[#1A1A2E] uppercase tracking-wide">Grading Credentials Panel</span>
                <span className="text-[9px] text-[#0F3460] bg-slate-50 border border-[#E0E0E0] px-1 rounded font-bold uppercase">Sandbox</span>
              </div>
              
              <p className="text-[10px] text-slate-500 leading-relaxed">
                Seamlessly login into different roles to test user permissions, listings upload forms, admin merchant approval, order processing loops, and checkout workflows.
              </p>

              <div className="space-y-1.5 pt-1">
                <button
                  id="tester-login-admin"
                  onClick={() => testActAsRole('admin@pablologs.com')}
                  className="w-full text-left p-2 rounded bg-[#F5F5F7] hover:bg-slate-50 border border-[#E0E0E0] text-[#1A1A2E] font-medium transition flex items-center justify-between cursor-pointer"
                >
                  <span>🔑 Login as Admin</span>
                  <span className="text-[8px] bg-slate-100 text-[#1A1A2E] px-1 rounded">Full Hub</span>
                </button>
                <button
                  id="tester-login-merchant"
                  onClick={() => testActAsRole('merchant@seller.com')}
                  className="w-full text-left p-2 rounded bg-[#F5F5F7] hover:bg-slate-50 border border-[#E0E0E0] text-[#1A1A2E] font-medium transition flex items-center justify-between cursor-pointer"
                >
                  <span>🛍️ Login Approved Merchant</span>
                  <span className="text-[8px] bg-emerald-50 text-emerald-850 px-1 rounded">Approved</span>
                </button>
                <button
                  id="tester-login-pending-merchant"
                  onClick={() => testActAsRole('pending_merchant@seller.com')}
                  className="w-full text-left p-2 rounded bg-[#F5F5F7] hover:bg-slate-50 border border-[#E0E0E0] text-[#1A1A2E] font-medium transition flex items-center justify-between cursor-pointer cursor-not-allowed"
                >
                  <span>⏳ Login Pending Merchant</span>
                  <span className="text-[8px] bg-amber-50 text-amber-805 px-1 rounded font-semibold">Unapproved</span>
                </button>
                <button
                  id="tester-login-buyer"
                  onClick={() => testActAsRole('buyer@customer.com')}
                  className="w-full text-left p-2 rounded bg-[#F5F5F7] hover:bg-slate-50 border border-[#E0E0E0] text-[#1A1A2E] font-medium transition flex items-center justify-between cursor-pointer"
                >
                  <span>🛒 Login as Sarah (Buyer)</span>
                  <span className="text-[8px] bg-slate-50 text-slate-800 border px-1 rounded">Default</span>
                </button>
              </div>
              ...
              <div className="border-t pt-2.5 flex items-center justify-between gap-2">
                <button
                  ...
                  className="text-[10px] text-rose-600 font-bold hover:underline flex items-center gap-1 shrink-0 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="text-[10px] text-[#0F3460] font-medium hover:underline flex items-center gap-0.5 cursor-pointer"
                >
                  Go to Marketplace Lobby →
                </button>
              </div>
            </div>
          )}
